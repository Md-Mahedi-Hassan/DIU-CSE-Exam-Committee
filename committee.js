...
function imageCandidates(initial) {
  const names = [...new Set([initial, initial.toUpperCase(), initial.toLowerCase()])].filter(Boolean);
  ...
  const filenames = names.flatMap((name) => extensions.map((extension) => extension ? `${name}.${extension}` : name));
  return [...new Set(folders.flatMap((folder) => filenames.map((filename) => `${folder}${filename}`)))];
}
...
async function loadCommittee() {
  try {
    ...
    grid.innerHTML = members.map((member) => {
      ...
      const firstImage = imageCandidates(memberInitial)[0];
        ...
        <div class="committee-card-head">
          <span class="committee-photo">
            <img
              ...
              data-initial="${escapeHtml(memberInitial)}"
              alt="${escapeHtml(member.name)}"
              title="Photo filename initial: ${escapeHtml(memberInitial)}"
              width="44"
              height="44"
              loading="eager"
              decoding="async"
    ...
    grid.querySelectorAll(".committee-photo img").forEach((image) => {
      const rawInitial = image.dataset.initial || "";
      const candidates = imageCandidates(rawInitial);
      ...
      const showFallback = () => {
        ...
        if (candidateIndex < candidates.length) {
          image.src = imageUrl(candidates[candidateIndex]);
          return;
        }
        image.parentElement.classList.remove("image-ready");
...
export { formatPhoneNumbers, imageCandidates, imageUrl, parseCommittee };
